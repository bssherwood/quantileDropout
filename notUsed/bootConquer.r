	...
	for( rho in rhos ){
		...
		for( m in ms ){
			...
			for( tau in taus ){
				...
				### Oracle Model
				coefs <- mapply(function(xx) conquer( XZbso[,-1], xx, tau=tau)$coeff, Yo, SIMPLIFY=FALSE)
				...
				for( jj in 1:nsim ){
					bootN[[jj]] <- t(mapply( function(xx,yy) conquer( xx[,-1], yy, tau=tau)$coeff[lin.coefs], XZbso.boot[[jj]], Yo.boot[[jj]] ))
				}
				...
				### Naive Model
				coefs <- mapply(function(xx,yy) conquer( xx[,-1], yy, tau=tau)$coeff, XZbsm, Ym, SIMPLIFY=FALSE)
				...
				bootN <- lapply(1:nsim, function(xx) 1)
				for( jj in 1:nsim ){
					bootN[[jj]] <- t(mapply( function(xx,yy) conquer( xx[,-1], yy, tau=tau)$coeff[lin.coefs], XZbsm.boot[[jj]], Ym.boot[[jj]] ))
				}
				CI <- lapply(bootN, function(xx) apply(xx, 2, quantile, probs=c(0.05, 0.95)) )
				width <- sapply(CI, function(xx) apply(xx, 2, diff) )
				coverage <- sapply(CI, function(xx) 1*(xx[1,]<=BTrue & BTrue<=xx[2,]) )
				...
				coefs <- mapply(function(xx,yy,zz){
									newx <- pmin(1/zz, 20)*xx
									newy <- pmin(1/zz, 20)*yy
									rq( newy~newx+0, tau=tau,method="conquer")$coefficients}, XZbsm, Ym, Pestm, SIMPLIFY=FALSE)
				...
				for( jj in 1:nsim ){
					bootN[[jj]] <- t(mapply( function(xx,yy,zz){
									newx <- pmin(1/zz, 20)*xx
									newy <- pmin(1/zz, 20)*yy
									rq( newy~newx+0, tau=tau,method="conquer")$coefficients[lin.coefs]}, XZbsm.boot[[jj]], Ym.boot[[jj]], Pestm.boot[[jj]] ))
				}
				CI <- lapply(bootN, function(xx) apply(xx, 2, quantile, probs=c(0.05, 0.95)) )
				width <- sapply(CI, function(xx) apply(xx, 2, diff) )
				coverage <- sapply(CI, function(xx) 1*(xx[1,]<=BTrue & BTrue<=xx[2,]) )
