...
bootN <- mapply(function(xx) boot.rq( XZbso, xx, tau=tau, R=bootNum,bsmethod="wild",method="conquer" )$B[,1:length(B)], Yo, SIMPLIFY=FALSE)

handBoot <- function(y,R,x,z1,z2,tau,bootNum){
	n <- dim(x)[1]
	p <- dim(x)[2]
	bootCoefs <- matrix(0,ncol=p,nrow=bootNum)
	for(i in 1:bootNum){
		...
		bootz2 <- z2[index]
		bootx <- x[index,]
		bootsplines <- cbind(bs(bootz1, degree=3, Boundary.knots=c(0,1)),bs(bootz2, degree=3, Boundary.knots=c(-1,1)))
...
for(ycoef in -1){
		...
		for( rho in rhos ){
			...
			for( m in ms ){
				...
				for( tau in taus ){
					...
					### Oracle Model
					coefs <- mapply(function(xx) rq.fit( XZbso, xx, tau=tau, method="conquer" )$coefficients, Yo, SIMPLIFY=FALSE)
					...
					bootN <- mapply(function(xx,yy) boot.rq( xx, yy, tau=tau, R=bootNum,bsmethod="xy" )$B[,1:length(B)], XZbsm, Ym, SIMPLIFY=FALSE)
					CI <- lapply(bootN, function(xx) apply(xx, 2, quantile, probs=c(0.05, 0.95)) )
					width <- sapply(CI, function(xx) apply(xx, 2, diff) )
					coverage <- sapply(CI, function(xx) 1*(xx[1,]<=BTrue & BTrue<=xx[2,]) )
					...
					bootN <- mapply(function(xx,yy,zz) boot.rq( pmin(1/zz, 20)*xx, pmin(1/zz, 20)*yy, tau=tau, R=bootNum,bsmethod="xy" )$B[,1:length(B)], XZbsm, Ym, Pestm, SIMPLIFY=FALSE)
					CI <- lapply(bootN, function(xx) apply(xx, 2, quantile, probs=c(0.05, 0.95)) )
					width <- sapply(CI, function(xx) apply(xx, 2, diff) )
					coverage <- sapply(CI, function(xx) 1*(xx[1,]<=BTrue & BTrue<=xx[2,]) )
	...
	# sd( gmse )/sqrt(nsim)
		...
		saveRDS( list(n=n, ms=ms, rhos=rhos, taus=taus, BTrue=BTrue, error=error, 
						...
						file=paste("NWildC_n_",n,"_ycoef_",ycoef,"_2022.RDS", sep="") )
...
saveRDS(outTotal, "goodAsymetricXYBig.RDS")
